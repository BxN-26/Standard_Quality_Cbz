...
try:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {executor.submit(process_image, file, resize_dimensions): file for file in image_files}
        for future in concurrent.futures.as_completed(futures):
            try:
                future.result()
            except Exception as e:
                print_error(f"Erreur lors du traitement de l'image {futures[future]} : {e}")
